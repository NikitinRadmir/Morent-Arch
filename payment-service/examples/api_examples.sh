...
curl -s -X POST "$BASE/transfers/batch" -H 'Content-Type: application/json' -H 'Idempotency-Key: demo-batch' -d "{\"items\":[{\"from_account_id\":\"$alice\",\"to_account_id\":\"$bob\",\"amount_minor\":500,\"currency\":\"RUB\"}]}"
